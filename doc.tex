...
\begin{center}
\begin{tikzpicture}[node distance=1.6cm, every node/.style={draw, text width=5cm, minimum height=1.2cm, align=center}, ->, >=stealth]
  \node (input) {Imagen original};
  \node (prep) [below=of input, fill=orange!20] {Preprocesamiento: filtros y texturas};
  \node (proc) [below=of prep] {Imagen procesada};
  \node (unet) [below=of proc, fill=yellow!20] {Red neuronal U-Net (entrenamiento o inferencia)};
  \node (mask) [below=of unet] {Máscara segmentada};
  \node (eval) [below=of mask, fill=purple!20] {Evaluación con métricas: Dice, IoU, etc.};
